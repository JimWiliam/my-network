...
with tf.Session() as sess:
    ckpt = tf.train.get_checkpoint_state('./model/SE_Inception_resnet_v2')
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
        saver.restore(sess, ckpt.model_checkpoint_path)
    else:
        sess.run(tf.global_variables_initializer())

    summary_writer = tf.summary.FileWriter('./logs/SE_Inception_resnet_v2', sess.graph)
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        # with open('/SE_Inception_resnet_v2/logs.txt', 'a') as f:
        #     f.write(line)

        saver.save(sess=sess, save_path='./model/SE_Inception_resnet_v2/Inception_resnet_v2.ckpt')
